...
io.on('connection', (socket) => {
    ...
    socket.on('devPlaceCard', ({ playerId, x, y, cardIndex }) => {
        if (!devModeEnabled) {
            socket.emit('error', 'Dev mode not enabled');
            return;
        }
    ...
    socket.on('devMoveUnit', ({ playerId, fromX, fromY, toX, toY }) => {
        if (!devModeEnabled) {
            socket.emit('error', 'Dev mode not enabled');
            return;
        }
